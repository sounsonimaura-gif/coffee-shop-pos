...
const form = useForm({
    ...
    adjustment_type: m.adjustment_type ?? 'increase',
    quantity: m.quantity ?? 1,
    reason: m.reason ?? '',
    note: m.note ?? '',
    adjusted_at: m.adjusted_at ?? new Date().toISOString().slice(0, 16),
});
...
<template>
    <AdminLayout
        :title="(isEdit ? t('edit') : t('create')) + ' — ' + t('stock_adjustments')"
        :page-title="(isEdit ? t('edit') : t('create')) + ' — ' + t('stock_adjustments')"
    >
        <form class="card" @submit.prevent="submit">
            <div class="card-header d-flex align-items-center justify-content-between">
                <h5 class="mb-0">{{ (isEdit ? t('edit') : t('create')) + ' — ' + t('stock_adjustments') }}</h5>
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        <FormField :label="t('adjustment_type')" :error="form.errors.adjustment_type">
                            <TomSelectField v-model="form.adjustment_type" :options="props.props.adjustment_types || []" value-field="value" label-field="text" />
                        </FormField>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        <FormField :label="t('adjusted_at')" :error="form.errors.adjusted_at">
                            <FlatpickrField v-model="form.adjusted_at" :options="{ enableTime: true }" />
                        </FormField>
                    </div>
                    <div class="col-md-3">
